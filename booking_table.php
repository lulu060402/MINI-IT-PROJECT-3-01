...
try {
    ...
    if (isset($_POST['room'], $_POST['phone'])) {
        // Process Laundry Booking Form
        
        // Use session name if available, otherwise use form input
        $name = isset($_SESSION['name']) ? $_SESSION['name'] : (isset($_POST['name']) ? $_POST['name'] : '');
        
        $required = ['room', 'phone', 'service_type', 'payment_method'];
        ...
        $stmt = $conn->prepare("INSERT INTO bookings 
            (username, room, phone, service_type, payment_method) 
            VALUES (?, ?, ?, ?, ?)");
        ...
        $runnerName = isset($_SESSION['name']) ? $_SESSION['name'] : (isset($_POST['runner-name']) ? $_POST['runner-name'] : '');
        $runnerEmail = isset($_SESSION['email']) ? $_SESSION['email'] : (isset($_POST['runner-email']) ? $_POST['runner-email'] : '');
        
        $required = ['runner-room'];
        foreach ($required as $field) {
            if (empty($_POST[$field])) {
                throw new Exception("Please fill in all required fields");
            }
        ...
        if (empty($runnerEmail)) {
            throw new Exception("Email is required");
        }
        
        $runnerData = [
            'name' => processInput($runnerName),
            'email' => processInput($runnerEmail, 'email'),
            'phone' => processInput($_POST['phone'], 'phone'),
